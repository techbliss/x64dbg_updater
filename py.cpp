...
#define module_name "X64Dbg_Update" //plugin name
...
static void Updater(); //edit this

extern "C" __declspec(dllexport) void CBMENUENTRY(CBTYPE cbType, PLUG_CB_MENUENTRY* info)
{
    switch(info->hEntry)
    {
	case MENU_UPDATER: // edit this
		Updater();
		break;
    }
}
...
// editor
static void Updater() // function to call main python app file
{
    wchar_t szFileName[MAX_PATH] = {0};
	char szDir[256]; GetCurrentDirectoryA(256,szDir); _plugin_logputs(szDir);
    ExecutePythonScript(L"plugins\\Updater\\x64dbg_updater.py"); // execution of main file location
}
...
void pyStop()
{
  //  _plugin_unregistercommand(pluginHandle, "Python");
   // _plugin_unregistercommand(pluginHandle, "OpenScript");
    _plugin_unregistercommand(pluginHandle, "Update x64dbg"); // edit this
...
void pySetup()
{
    ...
    FreeResource(hMem);
	_plugin_menuaddentry(hMenu, MENU_UPDATER, "x64dbg update"); // edit this (name in subwindow)
